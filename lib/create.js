...
/**
 * Will create a base Fcash Node configuration directory and files.
...
function createConfigDirectory(configDir, datadir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/fcash-node.json', configJSON);
      if (!isGlobal) {
...
/**
 * Will setup a directory with a Fcash Node directory, configuration file,
 * bitcoin configuration, and will install all necessary dependencies.
 *
 * @param {Object} options
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The name of the Fcash node configuration directory
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the Fcash-node directory and configuration
